...
- **Concurrent Calls:** `4,000` (Bidirectional bridging)
- **Hardware:** `16 Cores`
- **CPU Utilization:** `< 1%` per CPU Core (~23.5 MHz per core)
- **Memory Footprint:** `~1.05 GB` total fixed RSS (`~65 MB` per isolated core pool)
...
| `WS_PORT` | `8080` | The TCP port the WebSocket bridge listens on. |
...
Connect to the bridge via `ws://<WS_EXTERNAL_ADDR>:<WS_PORT>/?id=<INTERNAL_ID>`.
